...
export function WarehouseReceiveView() {
  ...
  return (
    <div className="max-w-3xl">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-semibold">Przyjęcie towaru</h1>
        <p className="text-sm text-gray-400 mt-1">Lokalizacja docelowa: <strong className="text-gray-700 font-medium">{warehouse?.name}</strong></p>
      </div>
      ...
      <div className="space-y-6">
        <section className={cardCls}>
          <h2 className={sectionTitleCls}>Dodaj pozycję do przyjęcia</h2>
        ...
        {items.length > 0 && (
          <section className={`${cardCls} p-0 overflow-hidden`}>
            <div className="px-5 pt-5 pb-2">
              <h2 className={sectionTitleCls}>Pozycje do przyjęcia ({items.length})</h2>
            </div>
        ...
        <div className="flex justify-end gap-2">
          ...
          <button
            onClick={submit}
            disabled={receive.isPending || items.length === 0}
          ...
          >
            {receive.isPending ? "Zapisywanie…" : "Zatwierdź przyjęcie"}
          </button>
        </div>
      </div>
    </div>
